...
from TexLiverNet_utils import *
from SFPM  import sfpms
from SFPM import SFPM_F
...
class DoubleConv1(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.initialiser = torch.rand((out_ch, 2))
        self.kwargs = {'encoding_layers': self.encoding_layers, 'initialiser': self.initialiser}
        self.conv = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        ...
        self.size = [256,256]
        # self.coor =  CoordGate(out_ch,out_ch,self.size,**self.kwargs)
        self.conv1 = nn.Sequential(
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class DoubleConv2(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.size = [128,128]
        # self.coor =  CoordGate(out_ch, out_ch, **self.kwargs)
        self.conv1 = nn.Sequential(
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class LiverNet(nn.Module):
    def __init__(self, num_classes, input_channels=1, deep_supervision=False, img_size=256,
                 embed_dims=[64, 128, 256, 512, 1024],
                 num_heads=[1, 2, 4, 8], qkv_bias=False, qk_scale=None, drop_rate=0.1, attn_drop_rate=0.,
        ...
        self.sfpms1 = sfpms(embed_dims[3], embed_dims[3])
        
        self.filter1 = SFPM_F(embed_dims[0], size=256)
        self.filter2 = SFPM_F(embed_dims[1], size=128)
        self.filter3 = SFPM_F(embed_dims[2], size=64)
        ...
        self.scale1 = nn.Parameter(torch.tensor(0.01),requires_grad=True)
        self.scale2 = nn.Parameter(torch.tensor(0.01),requires_grad=True)
        self.scale3 = nn.Parameter(torch.tensor(0.01),requires_grad=True)
    ...
    def forward(self, x):
        ...
        ### Conv Stage
        out = self.conv1(x)
        t1 = out + self.filter1(out) * self.scale1
       
        out = self.pool1(out)
        out = self.conv2(out)
        t2 = out + self.filter2(out) * self.scale2
        
        out = self.pool2(out)
        out = self.conv3(out)
        t3 = out + self.filter3(out) * self.scale3
        ...
        # print(self.scale)
        out = out + self.sfpms1(out) * self.scale
